...
3. **`gap_needs.json`** — if an edge has **zero** facing cams, append `recon/gap_needs.json` (do **not** invent MA peels). Hint: fetch far-side SV panos at fixed poses — see [`far-side-gap-needs.md`](far-side-gap-needs.md) (`ps1hood gap-needs fetch`).
